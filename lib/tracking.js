...
// ──────────────────────────────────────────────
//  🔍 Main: Detect → Create → Get
// ──────────────────────────────────────────────
async function trackWithTrackingMore(trackingNumber, carrier) {
  const apiKey = process.env.TM_API_KEY;

  // EMS ใช้ code ตรงๆ, Flash ใช้ auto-detect
  let courierCode = TM_CARRIER[carrier];
  if (!courierCode) {
    courierCode = await detectCourier(apiKey, trackingNumber);
    console.log('Detected courier:', courierCode);
  }
...
// ──────────────────────────────────────────────
//  🔎 Auto-detect courier จาก tracking number
//  POST /couriers/detect
// ──────────────────────────────────────────────
async function detectCourier(apiKey, trackingNumber) {
  ...
  console.log('TM detect:', JSON.stringify(json));

  const couriers = json?.data?.couriers || [];
  if (couriers.length === 0) {
    throw new Error('ไม่สามารถระบุบริษัทขนส่งได้ กรุณาตรวจสอบเลขพัสดุอีกครั้ง');
  }

  // เลือก courier แรกที่ match (highest confidence)
  return couriers[0].courier_code;
}
